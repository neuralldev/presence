...
class presence extends eqLogic {
    ...
    public function preSave() {
		foreach ($this->getConfiguration('modes') as $key => $value) {		
			...
			foreach ($value['action'] as $_action) {
				if($_action['cmd'] == '') throw new Exception('Veuillez renseigner toutes les actions');
			}	
			foreach ($value['action_exit'] as $_action_exit) {
				if($_action_exit['cmd'] == '') throw new Exception('Veuillez renseigner toutes les actions de sortie');
			}	
		}		
	...
	public function check_compare($_object_value, $_operande, $_prog_value){
		...
		switch ($_operande){
			case ">" :
				if($_object_value > $_prog_value) { return true;}
				break;
			case "<" :
				if($_object_value < $_prog_value) { return true;}
				break;
			...
			case "!=" :
				...
				break;
			case "~" :
				if(strstr($_object_value,$_prog_value)) { return true;}
				break;
			default :
				break;
	...
	public function simu_presence(){
		...
		foreach($simulation as $_simulation){
			...
			if($start[1] > 59) { $start[1] = $start[1] - 60; $start[0] = $start[0] + 1;}
			if($start[0] > 23) { $start[0] = 0;}
			
			
			$differe = rand(0,$_simulation['differe']);
			$stop[1] = $stop[1] + $differe;
			if($stop[1] > 59) { $stop[1] = $stop[1] - 60; $stop[0] = $stop[0] + 1;}
			if($stop[0] > 23) { $stop[0] = 0;}
